				...
				else 
				{
					if(identify_register)
					{
					...
					else if(!identify_register)
						{
							...
							intRegVal.add(temp_reg_reference,value_reg_reference);
						}
				}			
		...
		{			
			...
			else
			{
				...
				else if(!identify_register)
				{
					...
					fRegVal.add(temp_reg_reference,value_reg_reference);
				}
			}
		...
		while(scan.hasNext() && !pattern_matched)
			{
				...
				else
				{
					...
					else if(!identify_Memory_Location)
					{
						...
						Mod_Mem.add(temp_Memory_reference);
						MemCon.set(temp_Memory_reference,value_to_Memory_Location);	
